...
import {
  ...
  type MatchResponse, type PlayerUpdate, type PlayerSearchResult, type LeaderboardEntry, type SessionSummary, type FixtureSlot, type FixturesResponse,
} from '../api/client'
...
type Tab = 'roster' | 'events' | 'sessions' | 'submit' | 'matches' | 'register-player'
...
function DashboardInner() {
  ...
  return (
    <div className="space-y-6">
      ...
      {tab === 'matches' && user?.academy_id && <MatchesTab academyId={user.academy_id} />}
...
function RosterTab({ academyId }: { academyId: string }) {
  ...
  const [editForm, setEditForm] = useState<PlayerUpdate | null>(null)
  ...
  const editPlayerQ = useQuery({
    queryKey: ['player', editingPlayerId],
    queryFn: () => playersApi.get(editingPlayerId!),
    enabled: !!editingPlayerId,
  })

  const updatePlayerMut = useMutation({
    mutationFn: (body: PlayerUpdate) => playersApi.update(editingPlayerId!, body),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['academy-leaderboard', academyId] })
      qc.invalidateQueries({ queryKey: ['player', editingPlayerId] })
  ...
  useEffect(() => {
    if (editPlayerQ.data) {
      ...
      setEditForm({
        ...
        seeding_level: player.seeding_level,
        seeding_reference: player.seeding_reference,
        current_rating: player.current_rating,
        virtual_matches: player.virtual_matches,
      })
      setEditError(null)
    }
  }, [editPlayerQ.data])
          ...
          <tbody className="divide-y divide-gray-800">
            {q.data?.items.map(row => (
              <tr key={row.player_id} className="hover:bg-gray-900/50 transition-colors">
                ...
                <td className="px-4 py-3 text-gray-400 text-xs">{row.age_group ?? '—'}</td>
                <td className="px-4 py-3 text-gray-400">{row.rated_matches}</td>
                <WinPctCell row={row} />
                <TrendCell row={row} />
                <DominanceCell row={row} />
                <td className="px-4 py-3 text-gray-400 text-xs">{lastActive(row.last_match_date)}</td>
                <td className="px-4 py-3 text-gray-400 text-xs">
                ...
                <td className="px-4 py-3 text-right">
                  <button
                    type="button"
                    onClick={() => setEditingPlayerId(row.player_id)}
                    className="px-2 py-1 rounded-lg bg-blue-600 text-white text-xs font-semibold hover:bg-blue-500"
                  >
                    Edit
                  </button>
                </td>
            ...
            {editPlayerQ.isLoading ? (
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={e => setEditForm(prev => prev ? { ...prev, gender: e.target.value ? (e.target.value as 'MALE' | 'FEMALE') : null } : prev)}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm"
                    >
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setEditForm(prev => prev ? { ...prev, nationality: e.target.value } : prev)}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setEditForm(prev => prev ? { ...prev, guardian_name: e.target.value } : prev)}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setEditForm(prev => prev ? { ...prev, guardian_phone: e.target.value } : prev)}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={e => setEditForm(prev => prev ? { ...prev, contact_email: e.target.value } : prev)}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-400 mb-1">Seeding level</label>
                    <select
                      value={editForm.seeding_level ?? 'UNSEEDED'}
                      onChange={e => setEditForm(prev => prev ? { ...prev, seeding_level: e.target.value } : prev)}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm"
                    >
                  ...
                  <div>
                    <label className="block text-sm text-gray-400 mb-1">Seeding reference</label>
                    <input
                      type="text"
                      value={editForm.seeding_reference ?? ''}
                      onChange={e => setEditForm(prev => prev ? { ...prev, seeding_reference: e.target.value } : prev)}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm"
                    />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-400 mb-1">Current rating</label>
                    <input
                      type="number"
                      min={0}
                      value={editForm.current_rating ?? 0}
                      onChange={e => setEditForm(prev => prev ? { ...prev, current_rating: Number(e.target.value) } : prev)}
                      disabled={editPlayerQ.data?.rated_matches_completed !== 0}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm disabled:cursor-not-allowed disabled:bg-gray-800/70"
                    />
                  </div>
                  <div>
                    <label className="block text-sm text-gray-400 mb-1">Virtual matches</label>
                    <input
                      type="number"
                      min={0}
                      value={editForm.virtual_matches ?? 0}
                      onChange={e => setEditForm(prev => prev ? { ...prev, virtual_matches: Number(e.target.value) } : prev)}
                      disabled={editPlayerQ.data?.rated_matches_completed !== 0}
                      className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm disabled:cursor-not-allowed disabled:bg-gray-800/70"
                    />
                  </div>
                ...
                {editPlayerQ.data?.rated_matches_completed !== 0 && (
                  <div className="rounded-lg border border-yellow-600 bg-yellow-950/20 p-3 text-sm text-yellow-200">
...
function MatchesTab({ academyId }: { academyId: string }) {
  const [eventId, setEventId] = useState('')
  const [sessionId, setSessionId] = useState<string | null>(null)
  ...
  const visibleEvents = eventsQ.data?.items.filter(ev => {
    ...
    if (Array.isArray((ev as any).participating_academy_ids) && (ev as any).participating_academy_ids.includes(academyId)) return true
    if (Array.isArray((ev as any).participatingAcademies) && (ev as any).participatingAcademies.some((a: any) => a.academy_id === academyId)) return true
    return ev.scheduling_mode === 'INTRA_ACADEMY' && (!Array.isArray((ev as any).participating_academies) || (ev as any).participating_academies.length === 0)
  }) ?? []
  ...
  function openEditor(match: MatchResponse) {
    if (match.ratings_applied_at) {
      setApiError('Rated matches cannot be edited')
      return
    }
    setSelectedMatch(match)
    setForm({
      sets_won_a: String(match.sets_won_a),
  ...
  if (eventsQ.error) return <ErrorMsg message={(eventsQ.error as Error).message} />
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm text-gray-400 mb-1">Event</label>
          <select value={eventId} onChange={e => { setEventId(e.target.value); setSessionId(null); setSelectedMatch(null) }}
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm">
        ...
        <div>
          ...
          <select value={sessionId ?? ''} onChange={e => { setSessionId(e.target.value || null); setSelectedMatch(null) }}
            disabled={!eventId}
            className="w-full bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white text-sm">
            <option value="">Select session…</option>
            {sessionOptions.map(session => (
              <option key={session.session_id} value={session.session_id}>
                {session.session_date} · {session.match_format.replace(/_/g, ' ')}
              </option>
            ))}
          </select>
        </div>
